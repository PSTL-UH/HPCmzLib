...
namespace UsefulProteomicsDatabases
{
	...
	class ProteinDbLoader final
	{
		public:
			...
			/// Used to load the block variable in the LoadProteinXML function.
			/// done recursively due to limitations in the code.
			/// hopefully it doesn't create a stack overflow situation
			static void LoadProteinXMLHelper(tinyxml2::XMLNode *xml, std::vector<std::string> &modTYpesToExclude, std::unordered_map<std::string, Modification*> &unknownModifications, bool &isContaminant, const std::string &proteinDbLocation, std::vector<Protein*> &targets, ProteinXmlEntry *block);

			/// Used to read through GetPtmListFromProteinXml
			/// done recursively due to limitations in the code
			/// hopefully it doesn't create a stack overflow situaiton
			static void GetPtmListFromProteinXmlHelper(tinyxml2::XMLNode *xml, StringBuilder *storedKnownModificationsBuilder, std::regex *startingWhitespace, bool &leave);
		public:
